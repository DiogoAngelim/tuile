
        
  const { registerBlockType } = wp.blocks;
  const { RichText, MediaUpload, InspectorControls } = wp.blockEditor;
  const { Panel, PanelBody, PanelRow, TextareaControl } = wp.components;


        registerBlockType('wp/with-stacked-columns-on-mobile', {
            title: 'with stacked columns on mobile',
            icon: (<img src="data:image/jpeg;base64," />),
            category: 'common',
            attributes: {
  "contentHLa": {
    "type": "string",
    "default": "Users"
  },
  "contentscE": {
    "type": "string",
    "default": "A list of all the users in your account including their name, title, email and role."
  },
  "contentXHX": {
    "type": "string",
    "default": "Add user"
  },
  "contentpBE": {
    "type": "string",
    "default": "Name"
  },
  "contentbXD": {
    "type": "string",
    "default": "Title"
  },
  "contentKjb": {
    "type": "string",
    "default": "Email"
  },
  "contentDZM": {
    "type": "string",
    "default": "Role"
  },
  "contentvGc": {
    "type": "string",
    "default": "Edit"
  },
  "contentJbc": {
    "type": "string",
    "default": "\n            Lindsay Walton\n            "
  },
  "contentOWr": {
    "type": "string",
    "default": "Title"
  },
  "contentKzT": {
    "type": "string",
    "default": "Front-end Developer"
  },
  "contentJza": {
    "type": "string",
    "default": "Email"
  },
  "contenthPa": {
    "type": "string",
    "default": "lindsay.walton@example.com"
  },
  "contentoiX": {
    "type": "string",
    "default": "Front-end Developer"
  },
  "contentHtp": {
    "type": "string",
    "default": "lindsay.walton@example.com"
  },
  "contentYmO": {
    "type": "string",
    "default": "Member"
  },
  "contentMAp": {
    "type": "string",
    "default": "Edit"
  },
  "contentDmW": {
    "type": "string",
    "default": ", Lindsay Walton"
  }
},
            edit(props) {
            const { attributes, setAttributes } = props;

            return (
                <div>
                    <InspectorControls>
                    
    <Panel>
      


























































































































































































































































































































































































































































































































































































    </Panel>
                    </InspectorControls>

                    <div>
    <div>
        <div className="px-4 sm:px-6 lg:px-8">
            <div className="sm:flex sm:items-center">
                <div className="sm:flex-auto">
                     <h1 className="text-xl font-semibold text-gray-900"><RichText tagName="span" value={attributes.contentHLa} default="Users" onChange={ (newtext) =>  {
        setAttributes({ contentHLa: newtext });
      }}
    /></h1>

                    <p className="mt-2 text-sm text-gray-700">
                        <RichText tagName="span" value={attributes.contentscE} default="A list of all the users in your account including their name, title, email and role." onChange={ (newtext) => { setAttributes({ contentscE: newtext }); }} /></p>
                </div>
                <div className="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                    <button type="button" className="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">
                        <RichText tagName="span" value={attributes.contentXHX} default="Add user" onChange={ (newtext) => { setAttributes({ contentXHX: newtext }); }} /></button>
                </div>
            </div>
            <div className="-mx-4 mt-8 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded-lg">
                <table className="min-w-full divide-y divide-gray-300">
                    <thead className="bg-gray-50">
                        <tr>
                            <th scope="col" className="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">
                                <RichText tagName="span" value={attributes.contentpBE} default="Name" onChange={ (newtext) => { setAttributes({ contentpBE: newtext }); }} /></th>
                            <th scope="col" className="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">
                                <RichText tagName="span" value={attributes.contentbXD} default="Title" onChange={ (newtext) => { setAttributes({ contentbXD: newtext }); }} /></th>
                            <th scope="col" className="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 sm:table-cell">
                                <RichText tagName="span" value={attributes.contentKjb} default="Email" onChange={ (newtext) => { setAttributes({ contentKjb: newtext }); }} /></th>
                            <th scope="col" className="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">
                                <RichText tagName="span" value={attributes.contentDZM} default="Role" onChange={ (newtext) => { setAttributes({ contentDZM: newtext }); }} /></th>
                            <th scope="col" className="relative py-3.5 pl-3 pr-4 sm:pr-6"> <span className="sr-only"><RichText tagName="span" value={attributes.contentvGc} default="Edit" onChange={ (newtext) =>  {
        setAttributes({ contentvGc: newtext });
      }}
    /></span>

                            </th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-gray-200 bg-white">
                        <tr>
                            <td className="w-full max-w-0 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:w-auto sm:max-w-none sm:pl-6">
                                <RichText tagName="span" value={attributes.contentJbc} default="Lindsay Walton" onChange={ (newtext) => { setAttributes({ contentJbc: newtext }); }} />
                                    <dl className="font-normal lg:hidden"> <dt className="sr-only"><RichText tagName="span" value={attributes.contentOWr} default="Title" onChange={ (newtext) =>  {
        setAttributes({ contentOWr: newtext });
      }}
    /></dt>

                                        <dd className="mt-1 truncate text-gray-700">
                                            <RichText tagName="span" value={attributes.contentKzT} default="Front-end Developer" onChange={ (newtext) => { setAttributes({ contentKzT: newtext }); }} /></dd> <dt className="sr-only sm:hidden"><RichText tagName="span" value={attributes.contentJza} default="Email" onChange={ (newtext) =>  {
        setAttributes({ contentJza: newtext });
      }}
    /></dt>

                                        <dd className="mt-1 truncate text-gray-500 sm:hidden">
                                            <RichText tagName="span" value={attributes.contenthPa} default="lindsay.walton@example.com" onChange={ (newtext) => { setAttributes({ contenthPa: newtext }); }} /></dd>
                                    </dl>
                            </td>
                            <td className="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell">
                                <RichText tagName="span" value={attributes.contentoiX} default="Front-end Developer" onChange={ (newtext) => { setAttributes({ contentoiX: newtext }); }} /></td>
                            <td className="hidden px-3 py-4 text-sm text-gray-500 sm:table-cell">
                                <RichText tagName="span" value={attributes.contentHtp} default="lindsay.walton@example.com" onChange={ (newtext) => { setAttributes({ contentHtp: newtext }); }} /></td>
                            <td className="px-3 py-4 text-sm text-gray-500">
                                <RichText tagName="span" value={attributes.contentYmO} default="Member" onChange={ (newtext) => { setAttributes({ contentYmO: newtext }); }} /></td>
                            <td className="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6"> <span className="text-indigo-600 hover:text-indigo-900"><RichText tagName="span" value={attributes.contentMAp} default="Edit" onChange={ (newtext) =>  {
        setAttributes({ contentMAp: newtext });
      }}
    /><span className="sr-only"><RichText tagName="span" value={attributes.contentDmW} default=", Lindsay Walton" onChange={ (newtext) =>  {
        setAttributes({ contentDmW: newtext });
      }}
    /></span></span>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
                </div>
                );
            },
            save(props) {
            const { attributes } = props;

            return (
                <div>
    <div>
        <div class="px-4 sm:px-6 lg:px-8">
            <div class="sm:flex sm:items-center">
                <div class="sm:flex-auto">
                     <h1 class="text-xl font-semibold text-gray-900"><RichText.Content value={attributes.contentHLa} /></h1>

                    <p class="mt-2 text-sm text-gray-700">
                        <RichText.Content value={attributes.contentscE} /></p>
                </div>
                <div class="mt-4 sm:mt-0 sm:ml-16 sm:flex-none">
                    <button type="button" class="inline-flex items-center justify-center rounded-md border border-transparent bg-indigo-600 px-4 py-2 text-sm font-medium text-white shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 sm:w-auto">
                        <RichText.Content value={attributes.contentXHX} /></button>
                </div>
            </div>
            <div class="-mx-4 mt-8 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded-lg">
                <table class="min-w-full divide-y divide-gray-300">
                    <thead class="bg-gray-50">
                        <tr>
                            <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">
                                <RichText.Content value={attributes.contentpBE} /></th>
                            <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 lg:table-cell">
                                <RichText.Content value={attributes.contentbXD} /></th>
                            <th scope="col" class="hidden px-3 py-3.5 text-left text-sm font-semibold text-gray-900 sm:table-cell">
                                <RichText.Content value={attributes.contentKjb} /></th>
                            <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">
                                <RichText.Content value={attributes.contentDZM} /></th>
                            <th scope="col" class="relative py-3.5 pl-3 pr-4 sm:pr-6"> <span class="sr-only"><RichText.Content value={attributes.contentvGc} /></span>

                            </th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-gray-200 bg-white">
                        <tr>
                            <td class="w-full max-w-0 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:w-auto sm:max-w-none sm:pl-6">
                                <RichText.Content value={attributes.contentJbc} />
                                    <dl class="font-normal lg:hidden"> <dt class="sr-only"><RichText.Content value={attributes.contentOWr} /></dt>

                                        <dd class="mt-1 truncate text-gray-700">
                                            <RichText.Content value={attributes.contentKzT} /></dd> <dt class="sr-only sm:hidden"><RichText.Content value={attributes.contentJza} /></dt>

                                        <dd class="mt-1 truncate text-gray-500 sm:hidden">
                                            <RichText.Content value={attributes.contenthPa} /></dd>
                                    </dl>
                            </td>
                            <td class="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell">
                                <RichText.Content value={attributes.contentoiX} /></td>
                            <td class="hidden px-3 py-4 text-sm text-gray-500 sm:table-cell">
                                <RichText.Content value={attributes.contentHtp} /></td>
                            <td class="px-3 py-4 text-sm text-gray-500">
                                <RichText.Content value={attributes.contentYmO} /></td>
                            <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6"> <span class="text-indigo-600 hover:text-indigo-900"><RichText.Content value={attributes.contentMAp} /><span class="sr-only"><RichText.Content value={attributes.contentDmW} /></span></span>
                            </td>
                        </tr>
                    </tbody>
                </table>
            </div>
        </div>
    </div>
</div>
            );
            },
        });
        