      ...
      <PanelRow>
        <div>
          <TextareaControl
            ...
            value={ attributes.svgXPJ }
            onChange={ ( value ) => {
              setAttributes({ svgXPJ: value });
            } }
          />
        </div>
      </PanelRow>
    ...
    <div>
      ...
      <svg
         className="h-16 w-16 border border-gray-300 bg-white text-gray-300" preserveAspectRatio="none" stroke="currentColor" fill="none" viewBox="0 0 200 200" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgXPJ }}
        >
      </svg>
      
            </div>
            <div>
                 <h4 className="text-lg font-bold"><RichText tagName="span" value={attributes.contentVbs} default="Lorem ipsum" onChange={ (newtext) =>  {
        setAttributes({ contentVbs: newtext });
      }}
    /></h4>

                <p className="mt-1">
                    <RichText tagName="span" value={attributes.contentaXJ} default="Repudiandae sint consequuntur vel. Amet ut nobis explicabo numquam expedita quia omnis voluptatem. Minus quidem ipsam quia iusto." onChange={ (newtext) => { setAttributes({ contentaXJ: newtext }); }} /></p>
            </div>
        </div>
    </div>
</div>
    ...
    <div>
      ...
      <svg
        ...
        >
            ...
            <div>
                 <h4 class="text-lg font-bold"><RichText.Content value={attributes.contentVbs} /></h4>

                <p class="mt-1">
                    <RichText.Content value={attributes.contentaXJ} /></p>
            </div>
        </div>
    </div>
</div>
