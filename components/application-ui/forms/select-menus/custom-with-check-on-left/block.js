  ...
  const { Panel, PanelBody, PanelRow, TextareaControl } = wp.components;
        ...
        registerBlockType('wp/custom-with-check-on-left', {
      ...
      <PanelRow>
        <div>
          <TextareaControl
            ...
            value={ attributes.svgAtC }
            onChange={ ( value ) => {
              setAttributes({ svgAtC: value });
            } }
          />
        </div>
      </PanelRow>
      ...
      <PanelRow>
        <div>
          <TextareaControl
            label="SVG Content"
            help="Enter your SVG content..."
            value={ attributes.svgXQT }
            onChange={ ( value ) => {
              setAttributes({ svgXQT: value });
            } }
          />
        </div>
      </PanelRow>
    ...
    <div>
        <div>
            <label id="listbox-label" className="block text-sm font-medium text-gray-700">
                <RichText tagName="span" value={attributes.contentpST} default="Assigned to" onChange={ (newtext) => { setAttributes({ contentpST: newtext }); }} /></label>
            <div className="mt-1 relative">
                <button type="button" className="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label"> <span className="block truncate"><RichText tagName="span" value={attributes.contentCrc} default="Tom Cook" onChange={ (newtext) =>  {
        setAttributes({ contentCrc: newtext });
      }}
    /></span>
 <span className="absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
      ...
      <svg
         className="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgAtC }}
        >
      </svg>
                ...
                <ul className="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm" tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-option-3">
                    <li className="text-gray-900 cursor-default select-none relative py-2 pl-8 pr-4" id="listbox-option-0" role="option"> <span className="font-normal block truncate"><RichText tagName="span" value={attributes.contentSrv} default="Wade Cooper" onChange={ (newtext) =>  {
        setAttributes({ contentSrv: newtext });
      }}
    /></span>
 <span className="text-indigo-600 absolute inset-y-0 left-0 flex items-center pl-1.5">
      ...
      <svg
         className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgXQT }}
        >
      </svg>
      ...
      <svg
        ...
        >
                ...
                <ul class="absolute z-10 mt-1 w-full bg-white shadow-lg max-h-60 rounded-md py-1 text-base ring-1 ring-black ring-opacity-5 overflow-auto focus:outline-none sm:text-sm" tabindex="-1" role="listbox" aria-labelledby="listbox-label" aria-activedescendant="listbox-option-3">
                    <li class="text-gray-900 cursor-default select-none relative py-2 pl-8 pr-4" id="listbox-option-0" role="option"> <span class="font-normal block truncate"><RichText.Content value={attributes.contentSrv} /></span>
