    ...
    <div>
        <div>
            <label id="listbox-label" className="block text-sm font-medium text-gray-700">
                <RichText tagName="span" value={attributes.contentAop} default="Assigned to" onChange={ (newtext) => { setAttributes({ contentAop: newtext }); }} /></label>
            <div className="mt-1 relative">
                <button type="button" className="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label"> <span className="flex items-center">
        
      <MediaUpload 
        onSelect={ (media) => { 
          setAttributes({ 
            imageurlJKW: media.url,
            imagealtPSZ: media.alt
          }); 
        } }
        type="image" 
        render={ ({ open }) => (
          <img
            src={ attributes.imageurlJKW } 
            alt={ attributes.imagealtPSZ } 
            onClick={ open } 
            className="flex-shrink-0 h-6 w-6 rounded-full"
          /> 
        )} 
      />
        <span className="ml-3 block truncate"><RichText tagName="span" value={attributes.contentAgb} default="Tom Cook" onChange={ (newtext) =>  {
        setAttributes({ contentAgb: newtext });
      }}
    /></span>
</span> <span className="ml-3 absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
      ...
      <MediaUpload 
        onSelect={ (media) => { 
          setAttributes({ 
            imageurlmxq: media.url,
            imagealtFGd: media.alt
          }); 
        } }
        type="image" 
        render={ ({ open }) => (
          <img
            src={ attributes.imageurlmxq } 
            alt={ attributes.imagealtFGd } 
            onClick={ open } 
            className="flex-shrink-0 h-6 w-6 rounded-full"
          /> 
        )} 
      /> <span className="font-normal ml-3 block truncate"><RichText tagName="span" value={attributes.contentoyt} default="Wade Cooper" onChange={ (newtext) =>  {
    ...
    <div>
        <div>
            <label id="listbox-label" class="block text-sm font-medium text-gray-700">
                <RichText.Content value={attributes.contentAop} /></label>
            <div class="mt-1 relative">
                <button type="button" class="relative w-full bg-white border border-gray-300 rounded-md shadow-sm pl-3 pr-10 py-2 text-left cursor-default focus:outline-none focus:ring-1 focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm" aria-haspopup="listbox" aria-expanded="true" aria-labelledby="listbox-label"> <span class="flex items-center">
      ...
      <img
          ...
          />
        <span class="ml-3 block truncate"><RichText.Content value={attributes.contentAgb} /></span>
</span> <span class="ml-3 absolute inset-y-0 right-0 flex items-center pr-2 pointer-events-none">
      ...
      <img
            ...
            alt={ attributes.imagealtFGd } 
            class="flex-shrink-0 h-6 w-6 rounded-full"
          /> <span class="font-normal ml-3 block truncate"><RichText.Content value={attributes.contentoyt} /></span>
