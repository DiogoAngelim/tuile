  ...
  const { Panel, PanelBody, PanelRow, TextareaControl } = wp.components;
        ...
        registerBlockType('wp/with-status-indicator', {
      ...
      <PanelRow>
        <div>
          <TextareaControl
            ...
            value={ attributes.svgFWX }
            onChange={ ( value ) => {
              setAttributes({ svgFWX: value });
            } }
          />
        </div>
      </PanelRow>
      ...
      <PanelRow>
        <div>
          <TextareaControl
            label="SVG Content"
            help="Enter your SVG content..."
            value={ attributes.svgZsM }
            onChange={ ( value ) => {
              setAttributes({ svgZsM: value });
            } }
          />
        </div>
      </PanelRow>
    ...
    <div>
        <div>
            <label for="combobox" className="block text-sm font-medium text-gray-700">
                <RichText tagName="span" value={attributes.contentXfo} default="Assigned to" onChange={ (newtext) => { setAttributes({ contentXfo: newtext }); }} /></label>
      ...
      <svg
         className="h-5 w-5 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgFWX }}
        >
      </svg>
                ...
                <ul className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm" id="options" role="listbox">
                    <li className="relative cursor-default select-none py-2 pl-3 pr-9 text-gray-900" id="option-0" role="option" tabindex="-1">
                        <div className="flex items-center"> <span className="inline-block h-2 w-2 flex-shrink-0 rounded-full bg-green-400" aria-hidden="true"/>
 <span className="ml-3 truncate"><RichText tagName="span" value={attributes.contentuxh} default="Leslie Alexander" onChange={ (newtext) =>  {
        setAttributes({ contentuxh: newtext });
      }}
    /><span className="sr-only"><RichText tagName="span" value={attributes.contentTgi} default="is online" onChange={ (newtext) =>  {
        setAttributes({ contentTgi: newtext });
      }}
    /></span>
</span>
      ...
      <svg
         className="h-5 w-5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgZsM }}
        >
      </svg>
    ...
    <div>
        <div>
            <label for="combobox" class="block text-sm font-medium text-gray-700">
                <RichText.Content value={attributes.contentXfo} /></label>
        ...
        >
                ...
                <ul class="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm" id="options" role="listbox">
                    <li class="relative cursor-default select-none py-2 pl-3 pr-9 text-gray-900" id="option-0" role="option" tabindex="-1">
                        <div class="flex items-center"> <span class="inline-block h-2 w-2 flex-shrink-0 rounded-full bg-green-400" aria-hidden="true"/>
 <span class="ml-3 truncate"><RichText.Content value={attributes.contentuxh} /><span class="sr-only"><RichText.Content value={attributes.contentTgi} /></span>
</span>
