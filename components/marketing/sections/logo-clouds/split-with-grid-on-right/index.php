    ...
    <?php
    /*
      * Plugin Name:       split_with_grid_on_right
      * Version:           1.0
    ...
    */
    ...
    add_action( 'enqueue_block_editor_assets', 'wpfuncabz_split_with_grid_on_rightfuncEBE_editor_assets' );
      ...
      wp_enqueue_script(
        'wp-split-with-grid-on-right',
        plugins_url( 'block.build.js', __FILE__ ),
        array( 'wp-blocks', 'wp-components', 'wp-element' ,'wp-editor'),
        $version
      );

      wp_localize_script( 'wp-split-with-grid-on-right', 'vars', array( 'url' => plugin_dir_url( __FILE__ ) ) );

      $filepath = plugin_dir_path(__FILE__) . 'editor.css';
      $version = file_exists($filepath) ? filemtime($filepath) : time();

      wp_enqueue_style(
        'wp-split-with-grid-on-right-editor',
    ...
    function wpfuncabz_split_with_grid_on_rightfuncEBE_block_assets() {
      ...
      wp_enqueue_script(
        'wp-split-with-grid-on-right-js',
        plugins_url( 'scripts.js', __FILE__ ),
        array(),
        $version
      );

      wp_localize_script( 'wp-split-with-grid-on-right-js', 'vars', array( 'url' => plugin_dir_url( __FILE__ ) ) );
    }
