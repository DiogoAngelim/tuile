    ...
    */
    ...
    add_action( 'enqueue_block_editor_assets', 'wpfuncfHW_split_brand_panelfuncmez_editor_assets' );
      ...
      wp_enqueue_script(
        'wp-split-brand-panel',
        plugins_url( 'block.build.js', __FILE__ ),
        array( 'wp-blocks', 'wp-components', 'wp-element' ,'wp-editor'),
        $version
      );

      wp_localize_script( 'wp-split-brand-panel', 'vars', array( 'url' => plugin_dir_url( __FILE__ ) ) );

      $filepath = plugin_dir_path(__FILE__) . 'editor.css';
      $version = file_exists($filepath) ? filemtime($filepath) : time();

      wp_enqueue_style(
        'wp-split-brand-panel-editor',
    ...
    function wpfuncfHW_split_brand_panelfuncmez_block_assets() {
      ...
      wp_enqueue_script(
        'wp-split-brand-panel-js',
        plugins_url( 'scripts.js', __FILE__ ),
        array(),
        $version
      );

      wp_localize_script( 'wp-split-brand-panel-js', 'vars', array( 'url' => plugin_dir_url( __FILE__ ) ) );
    }
