
        
  const { registerBlockType } = wp.blocks;
  const { RichText, MediaUpload, InspectorControls } = wp.blockEditor;
  const { Panel, PanelBody, PanelRow, TextareaControl } = wp.components;


        registerBlockType('wp/simple-with-select-menu', {
            title: 'simple with select menu',
            icon: (<img src="data:image/jpeg;base64," />),
            category: 'common',
            attributes: {
  "contentwjj": {
    "type": "string",
    "default": "Pricing Plans"
  },
  "contentkYj": {
    "type": "string",
    "default": "Start building for free, then add a site plan to go live. Account plans unlock additional features."
  },
  "contentJRe": {
    "type": "string",
    "default": "Currency"
  },
  "contentsmX": {
    "type": "string",
    "default": "Argentina (ARS)"
  },
  "contentpSn": {
    "type": "string",
    "default": "Australia (AUD)"
  },
  "contentZha": {
    "type": "string",
    "default": "United States (USD)"
  },
  "contentXSG": {
    "type": "string",
    "default": "Canada (CAD)"
  },
  "contentgth": {
    "type": "string",
    "default": "France (EUR)"
  },
  "contentWIr": {
    "type": "string",
    "default": "Japan (JPY)"
  },
  "contentcVt": {
    "type": "string",
    "default": "Nigeria (NGN)"
  },
  "contentFnL": {
    "type": "string",
    "default": "Switzerland (CHF)"
  },
  "contentuPg": {
    "type": "string",
    "default": "United Kingdom (GBP)"
  },
  "svgsDk": {
    "type": "string",
    "default": "<path fill-rule=\"evenodd\" d=\"M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z\" clip-rule=\"evenodd\"/>"
  }
},
            edit(props) {
            const { attributes, setAttributes } = props;

            return (
                <div>
                    <InspectorControls>
                    
    <Panel>
      

































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































































    { (            
    <PanelBody title="SVG Markup">
      <PanelRow>
        <div>
          <TextareaControl
            label="SVG Content"
            help="Enter your SVG content..."
            value={ attributes.svgsDk }
            onChange={ ( value ) => {
              setAttributes({ svgsDk: value });
            } }
          />
        </div>
      </PanelRow>
    </PanelBody>
    )}
  
    </Panel>
                    </InspectorControls>

                    <div>
    <div>
        <div className="bg-white">
            <div className="max-w-7xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:px-8 lg:flex lg:justify-between">
                <div className="max-w-xl">
                     <h2 className="text-4xl font-extrabold text-gray-900 sm:text-5xl sm:tracking-tight lg:text-6xl"><RichText tagName="span" value={attributes.contentwjj} default="Pricing Plans" onChange={ (newtext) =>  {
        setAttributes({ contentwjj: newtext });
      }}
    /></h2>

                    <p className="mt-5 text-xl text-gray-500">
                        <RichText tagName="span" value={attributes.contentkYj} default="Start building for free, then add a site plan to go live. Account plans unlock additional features." onChange={ (newtext) => { setAttributes({ contentkYj: newtext }); }} /></p>
                </div>
                <div className="mt-10 w-full max-w-xs">
                    <label for="currency" className="block text-base font-medium text-gray-500">
                        <RichText tagName="span" value={attributes.contentJRe} default="Currency" onChange={ (newtext) => { setAttributes({ contentJRe: newtext }); }} /></label>
                    <div className="mt-1.5 relative">
                        <select id="currency" name="currency" className="appearance-none block w-full bg-none bg-white border border-gray-300 rounded-md pl-3 pr-10 py-2 text-base text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            <option>
                                <RichText tagName="span" value={attributes.contentsmX} default="Argentina (ARS)" onChange={ (newtext) => { setAttributes({ contentsmX: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentpSn} default="Australia (AUD)" onChange={ (newtext) => { setAttributes({ contentpSn: newtext }); }} /></option>
                            <option selected="">
                                <RichText tagName="span" value={attributes.contentZha} default="United States (USD)" onChange={ (newtext) => { setAttributes({ contentZha: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentXSG} default="Canada (CAD)" onChange={ (newtext) => { setAttributes({ contentXSG: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentgth} default="France (EUR)" onChange={ (newtext) => { setAttributes({ contentgth: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentWIr} default="Japan (JPY)" onChange={ (newtext) => { setAttributes({ contentWIr: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentcVt} default="Nigeria (NGN)" onChange={ (newtext) => { setAttributes({ contentcVt: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentFnL} default="Switzerland (CHF)" onChange={ (newtext) => { setAttributes({ contentFnL: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentuPg} default="United Kingdom (GBP)" onChange={ (newtext) => { setAttributes({ contentuPg: newtext }); }} /></option>
                        </select>
                        <div className="pointer-events-none absolute inset-y-0 right-0 px-2 flex items-center">
                            
      <svg
         className="h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgsDk }}
        >
      </svg>
      
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
                </div>
                );
            },
            save(props) {
            const { attributes } = props;

            return (
                <div>
    <div>
        <div class="bg-white">
            <div class="max-w-7xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:px-8 lg:flex lg:justify-between">
                <div class="max-w-xl">
                     <h2 class="text-4xl font-extrabold text-gray-900 sm:text-5xl sm:tracking-tight lg:text-6xl"><RichText.Content value={attributes.contentwjj} /></h2>

                    <p class="mt-5 text-xl text-gray-500">
                        <RichText.Content value={attributes.contentkYj} /></p>
                </div>
                <div class="mt-10 w-full max-w-xs">
                    <label for="currency" class="block text-base font-medium text-gray-500">
                        <RichText.Content value={attributes.contentJRe} /></label>
                    <div class="mt-1.5 relative">
                        <select id="currency" name="currency" class="appearance-none block w-full bg-none bg-white border border-gray-300 rounded-md pl-3 pr-10 py-2 text-base text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            <option>
                                <RichText.Content value={attributes.contentsmX} /></option>
                            <option>
                                <RichText.Content value={attributes.contentpSn} /></option>
                            <option selected="">
                                <RichText.Content value={attributes.contentZha} /></option>
                            <option>
                                <RichText.Content value={attributes.contentXSG} /></option>
                            <option>
                                <RichText.Content value={attributes.contentgth} /></option>
                            <option>
                                <RichText.Content value={attributes.contentWIr} /></option>
                            <option>
                                <RichText.Content value={attributes.contentcVt} /></option>
                            <option>
                                <RichText.Content value={attributes.contentFnL} /></option>
                            <option>
                                <RichText.Content value={attributes.contentuPg} /></option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 px-2 flex items-center">
                            
      <svg
         class="h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgsDk }}
        >
      </svg>
      
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
            );
            },
        });
        