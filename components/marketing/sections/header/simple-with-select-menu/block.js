
        
  const { registerBlockType } = wp.blocks;
  const { RichText, MediaUpload, InspectorControls } = wp.blockEditor;
  const { Panel, PanelBody, PanelRow, TextareaControl, ToggleControl } = wp.components;


        registerBlockType('wp/simple-with-select-menu', {
            title: 'simple-with-select-menu',
            icon: (<img src="data:image/jpeg;base64," />),
            category: 'common',
            attributes: {
  "contentBiq": {
    "type": "string",
    "default": "Pricing Plans"
  },
  "contentDjK": {
    "type": "string",
    "default": "Start building for free, then add a site plan to go live. Account plans unlock additional features."
  },
  "contentrTV": {
    "type": "string",
    "default": "Currency"
  },
  "contentmdA": {
    "type": "string",
    "default": "Argentina (ARS)"
  },
  "contentpUi": {
    "type": "string",
    "default": "Australia (AUD)"
  },
  "contentLNB": {
    "type": "string",
    "default": "United States (USD)"
  },
  "contentjWu": {
    "type": "string",
    "default": "Canada (CAD)"
  },
  "contentcDX": {
    "type": "string",
    "default": "France (EUR)"
  },
  "contentugT": {
    "type": "string",
    "default": "Japan (JPY)"
  },
  "contentJlF": {
    "type": "string",
    "default": "Nigeria (NGN)"
  },
  "contentHNX": {
    "type": "string",
    "default": "Switzerland (CHF)"
  },
  "contentYJm": {
    "type": "string",
    "default": "United Kingdom (GBP)"
  },
  "svgEhY": {
    "type": "string",
    "default": "<path fill-rule=\"evenodd\" d=\"M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z\" clip-rule=\"evenodd\"/>"
  }
},
            edit(props) {
            const { attributes, setAttributes } = props;

            return (
                <div>
                    <InspectorControls>
                    
    <Panel>
      



































































































































    { (            
    <PanelBody title="SVG Markup">
      <PanelRow>
        <div>
          <TextareaControl
            label="SVG Content"
            help="Enter your SVG content..."
            value={ attributes.svgEhY }
            onChange={ ( value ) => {
              setAttributes({ svgEhY: value });
            } }
          />
        </div>
      </PanelRow>
    </PanelBody>
    )}
  
    </Panel>
                    </InspectorControls>

                    <div>
    <div>
        <div className="bg-white">
            <div className="max-w-7xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:px-8 lg:flex lg:justify-between">
                <div className="max-w-xl">
                     <h2 className="text-4xl font-extrabold text-gray-900 sm:text-5xl sm:tracking-tight lg:text-6xl"><RichText tagName="span" value={attributes.contentBiq} default="Pricing Plans" onChange={ (newtext) =>  {
        setAttributes({ contentBiq: newtext });
      }}
    /></h2>

                    <p className="mt-5 text-xl text-gray-500">
                        <RichText tagName="span" value={attributes.contentDjK} default="Start building for free, then add a site plan to go live. Account plans unlock additional features." onChange={ (newtext) => { setAttributes({ contentDjK: newtext }); }} /></p>
                </div>
                <div className="mt-10 w-full max-w-xs">
                    <label for="currency" className="block text-base font-medium text-gray-500">
                        <RichText tagName="span" value={attributes.contentrTV} default="Currency" onChange={ (newtext) => { setAttributes({ contentrTV: newtext }); }} /></label>
                    <div className="mt-1.5 relative">
                        <select id="currency" name="currency" className="appearance-none block w-full bg-none bg-white border border-gray-300 rounded-md pl-3 pr-10 py-2 text-base text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            <option>
                                <RichText tagName="span" value={attributes.contentmdA} default="Argentina (ARS)" onChange={ (newtext) => { setAttributes({ contentmdA: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentpUi} default="Australia (AUD)" onChange={ (newtext) => { setAttributes({ contentpUi: newtext }); }} /></option>
                            <option selected="">
                                <RichText tagName="span" value={attributes.contentLNB} default="United States (USD)" onChange={ (newtext) => { setAttributes({ contentLNB: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentjWu} default="Canada (CAD)" onChange={ (newtext) => { setAttributes({ contentjWu: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentcDX} default="France (EUR)" onChange={ (newtext) => { setAttributes({ contentcDX: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentugT} default="Japan (JPY)" onChange={ (newtext) => { setAttributes({ contentugT: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentJlF} default="Nigeria (NGN)" onChange={ (newtext) => { setAttributes({ contentJlF: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentHNX} default="Switzerland (CHF)" onChange={ (newtext) => { setAttributes({ contentHNX: newtext }); }} /></option>
                            <option>
                                <RichText tagName="span" value={attributes.contentYJm} default="United Kingdom (GBP)" onChange={ (newtext) => { setAttributes({ contentYJm: newtext }); }} /></option>
                        </select>
                        <div className="pointer-events-none absolute inset-y-0 right-0 px-2 flex items-center">
                            
      <svg
         className="h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgEhY }}
        >
      </svg>
      
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
                </div>
                );
            },
            save(props) {
            const { attributes } = props;

            return (
                <div>
    <div>
        <div class="bg-white">
            <div class="max-w-7xl mx-auto py-16 px-4 sm:py-24 sm:px-6 lg:px-8 lg:flex lg:justify-between">
                <div class="max-w-xl">
                     <h2 class="text-4xl font-extrabold text-gray-900 sm:text-5xl sm:tracking-tight lg:text-6xl"><RichText.Content value={attributes.contentBiq} /></h2>

                    <p class="mt-5 text-xl text-gray-500">
                        <RichText.Content value={attributes.contentDjK} /></p>
                </div>
                <div class="mt-10 w-full max-w-xs">
                    <label for="currency" class="block text-base font-medium text-gray-500">
                        <RichText.Content value={attributes.contentrTV} /></label>
                    <div class="mt-1.5 relative">
                        <select id="currency" name="currency" class="appearance-none block w-full bg-none bg-white border border-gray-300 rounded-md pl-3 pr-10 py-2 text-base text-gray-900 focus:outline-none focus:ring-indigo-500 focus:border-indigo-500 sm:text-sm">
                            <option>
                                <RichText.Content value={attributes.contentmdA} /></option>
                            <option>
                                <RichText.Content value={attributes.contentpUi} /></option>
                            <option selected="">
                                <RichText.Content value={attributes.contentLNB} /></option>
                            <option>
                                <RichText.Content value={attributes.contentjWu} /></option>
                            <option>
                                <RichText.Content value={attributes.contentcDX} /></option>
                            <option>
                                <RichText.Content value={attributes.contentugT} /></option>
                            <option>
                                <RichText.Content value={attributes.contentJlF} /></option>
                            <option>
                                <RichText.Content value={attributes.contentHNX} /></option>
                            <option>
                                <RichText.Content value={attributes.contentYJm} /></option>
                        </select>
                        <div class="pointer-events-none absolute inset-y-0 right-0 px-2 flex items-center">
                            
      <svg
         class="h-4 w-4 text-gray-400" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true"
          dangerouslySetInnerHTML={ { __html: attributes.svgEhY }}
        >
      </svg>
      
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
            );
            },
        });
        