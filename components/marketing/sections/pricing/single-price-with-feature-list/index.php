    ...
    <?php
    /*
      * Plugin Name:       single_price_with_feature_list
      * Version:           1.0
    ...
    */
    ...
    add_action( 'enqueue_block_editor_assets', 'wpfuncPCA_single_price_with_feature_listfuncaTV_editor_assets' );
      ...
      wp_enqueue_script(
        'wp-single-price-with-feature-list',
        plugins_url( 'block.build.js', __FILE__ ),
        array( 'wp-blocks', 'wp-components', 'wp-element' ,'wp-editor'),
        $version
      );

      wp_localize_script( 'wp-single-price-with-feature-list', 'vars', array( 'url' => plugin_dir_url( __FILE__ ) ) );

      $filepath = plugin_dir_path(__FILE__) . 'editor.css';
      $version = file_exists($filepath) ? filemtime($filepath) : time();

      wp_enqueue_style(
        'wp-single-price-with-feature-list-editor',
    ...
    function wpfuncPCA_single_price_with_feature_listfuncaTV_block_assets() {
      ...
      wp_enqueue_script(
        'wp-single-price-with-feature-list-js',
        plugins_url( 'scripts.js', __FILE__ ),
        array(),
        $version
      );

      wp_localize_script( 'wp-single-price-with-feature-list-js', 'vars', array( 'url' => plugin_dir_url( __FILE__ ) ) );
    }
