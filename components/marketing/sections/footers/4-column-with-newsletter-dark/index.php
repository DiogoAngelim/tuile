    ...
    <?php
    /*
      * Plugin Name:       4_column_with_newsletter_dark
      * Version:           1.0
    ...
    */
    ...
    add_action( 'enqueue_block_editor_assets', 'wpfuncadh_4_column_with_newsletter_darkfuncLQo_editor_assets' );
      ...
      wp_enqueue_script(
        'wp-4-column-with-newsletter-dark',
        plugins_url( 'block.build.js', __FILE__ ),
        array( 'wp-blocks', 'wp-components', 'wp-element' ,'wp-editor'),
        $version
      );

      wp_localize_script( 'wp-4-column-with-newsletter-dark', 'vars', array( 'url' => plugin_dir_url( __FILE__ ) ) );

      $filepath = plugin_dir_path(__FILE__) . 'editor.css';
      $version = file_exists($filepath) ? filemtime($filepath) : time();

      wp_enqueue_style(
        'wp-4-column-with-newsletter-dark-editor',
    ...
    function wpfuncadh_4_column_with_newsletter_darkfuncLQo_block_assets() {
      ...
      wp_enqueue_script(
        'wp-4-column-with-newsletter-dark-js',
        plugins_url( 'scripts.js', __FILE__ ),
        array(),
        $version
      );

      wp_localize_script( 'wp-4-column-with-newsletter-dark-js', 'vars', array( 'url' => plugin_dir_url( __FILE__ ) ) );
    }
